...
function TreeNode(container) {
  ...
  this.element.oncontextmenu = function (event) {
    if (!self.object.locked) {
      ...
      if (self.object instanceof THREE.Mesh || self.object instanceof THREE.SkinnedMesh) {
        //if mesh has material you  can edit material
        if (self.object.material != undefined) {
          context.addOption(Locale.editMaterial, function () {
            let materialAsset = Editor.gui.assetExplorer.assets.parent.files.find(
              (item) => item.asset.uuid == self.object.material.uuid
            );

            if (materialAsset) {
              openMaterialEditor(materialAsset);
            }
          });
        }
        ...
        if (self.object.geometry !== undefined) {
          ...
          context.addOption(Locale.applyTransformation, function () {
            ...
            actions.push(
              new ChangeAction(self.object, 'quaternion', new THREE.Quaternion(0, 0, 0, 1))
            );
  ...
  this.element.ondrop = function (event) {
    ...
    if (object instanceof THREE.Object3D) {
      if (object === self.object) {
        ...
        //Inside //if(dragState === TreeNode.INSIDE)
        else {
          if (
            (selfIsScene && !dragIsScene) ||
            (dragIsScene && selfIsProgram) ||
            (!selfIsScene && !selfIsProgram && !dragIsScene)
          ) {
  ...
  this.element.onclick = function (event) {
    if (
      ...
      Editor.selection[Editor.selection.length - 1].isObject3D === true
    ) {
...
TreeNode.prototype.setBorder = function (place) {
  ...
  } //if(place === TreeNode.INSIDE)
  else {
    this.element.style.border = '1px solid #999999';
  }
};
/**
 * LabelText.data=Object3D（material.name）
 * @method getObjName
