...
Editor.initialize = function () {
  ...
  Editor.ternDefinitions.push(
    JSON.parse(FileSystem.readFile(Global.FILE_PATH + 'tern/threejs.json'))
  );
  Editor.ternDefinitions.push(
    JSON.parse(FileSystem.readFile(Global.FILE_PATH + 'tern/browser.json'))
  );
  Editor.ternDefinitions.push(
    JSON.parse(FileSystem.readFile(Global.FILE_PATH + 'tern/ecmascript.json'))
  );
  ...
  if (Nunu.runningOnDesktop()) {
    ...
    document.onkeydown = function (event) {
      ...
      if (
        (event.keyCode > Keyboard.F1 && event.keyCode < Keyboard.F11) ||
        (!event.altKey && event.ctrlKey && allowedKeys.indexOf(event.keyCode) === -1)
      ) {
...
Editor.createDefaultResouces = function () {
  ...
  Editor.defaultSpriteMaterial = new THREE.SpriteMaterial({
    map: Editor.defaultTexture,
    color: 0xffffff
  });
...
Editor.setOpenFile = function (file) {
  if (file !== undefined && file !== null) {
    ...
    document.title =
      Nunu.NAME + ' ' + Nunu.VERSION + ' (' + Nunu.TIMESTAMP + ') (' + Editor.openFile + ')';
  } else {
    Editor.openFile = null;
...
Editor.updateNunu = function (silent) {
  ...
  try {
    var url =
      'https://raw.githubusercontent.com/tentone/nunuStudio/master/build/nunu.editor.min.js';
