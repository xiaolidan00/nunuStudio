 ...
 */
ObjectLoader.prototype.parseObject = function (
  data,
  geometries,
  materials,
  textures,
  ...
  switch (data.type) {
    ...
    case 'Physics':
      ...
      for (var i = 0; i < shapes.length; i++) {
        ...
        if (shape.type === CANNON.Shape.types.SPHERE) {
          ...
          object.body.addShape(
            new CANNON.Box(
              new CANNON.Vec3(shape.halfExtents.x, shape.halfExtents.y, shape.halfExtents.z)
            )
          );
    ...
    case 'LensFlare':
      ...
      for (var i = 0; i < data.elements.length; i++) {
        object.addFlare(
          getTexture(data.elements[i].texture),
          data.elements[i].size,
          data.elements[i].distance,
          new THREE.Color(data.elements[i].color)
        );
      }
  ...
  if (data.children !== undefined) {
    for (var child in data.children) {
      object.add(
        this.parseObject(data.children[child], geometries, materials, textures, audio, fonts)
      );
    }
  }
